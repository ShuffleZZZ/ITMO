...
public class ArticleRepositoryImpl implements ArticleRepository {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't add article.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't add article.", e);
        }
    }
    ...
    public List<FrontArticle> findAllShowed() {
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        result.add(new FrontArticle(toArticle(statement.getMetaData(), resultSet)));
                    }
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't find showing articles.", e);
        }
    ...
    public void setShowing(Long articleId, boolean state) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    ...
                    Statement.RETURN_GENERATED_KEYS)) {
                statement.setString(1, Integer.toString(state ? 0 : 1));
